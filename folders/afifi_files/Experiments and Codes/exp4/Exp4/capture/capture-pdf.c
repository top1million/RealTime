...
unsigned int  rising_edge_time=0; // hold the current value of rising edge time 
char str[10] ;
...
void main(void) {
	...
	TMR1CS = 0; //use internal clock (Fosc/4) , where Fosc = 4MHz in our case
	TMR1H = 0x00;
	TMR1L = 0x00;	//TMR1H and TMR1L are high and low registers of timer 1
	T1CKPS0 = 0;
	T1CKPS1 = 0;	//set theprescale value to 1:1 (refer to datasheet)
	T1SYNC = 0;	//timer is synchronised to external clock input
...
static void interrupt ISR(void) {
  ...
  if ( CCP1IF ) {
	...
	CCP1IF = 0; //reset the ccp1 interrupt flag to 0
  }
